...
describe("computeWeekVariance", () => {
  ...
  it("(#791) includes Amex and other Plaid card rows alongside Chase, excludes transfers", async () => {
    // Set up two Plaid items/accounts: a Chase checking and an Amex
    // card. Prior to #791, only the checking account's rows would
    // count toward Debrief actuals and Amex would be hard-excluded.
    const [chaseItem] = await db
      ...
      .values({
        ...
        accessToken: "test-chase",
        institutionSlug: "chase",
        cursor: "c0",
      })
      .returning();
    const chaseExternal = `acct-chase-${randomUUID()}`;
    ...
    const [amexItem] = await db
      ...
      .values({
        ...
        accessToken: "test-amex",
        institutionSlug: "amex",
        cursor: "c0",
      })
      .returning();
    const amexExternal = `acct-amex-${randomUUID()}`;
    await db.insert(plaidAccountsTable).values({
      userId: TEST_USER,
      householdId: TEST_HOUSEHOLD_ID,
      itemId: amexItem!.id,
      accountId: amexExternal,
      name: "Amex Card",
      type: "credit",
      subtype: "credit card",
    });
    // Configured checking points at Chase — historically would have
    // gated everything to Chase only.
    await db.insert(forecastSettingsTable).values({
      userId: TEST_USER,
      householdId: TEST_HOUSEHOLD_ID,
    ...
    // Categories: a normal Groceries bucket (Chase + Amex both
    // charge to it) and a system-excluded Transfer bucket (#783
    // filter must still drop these even though the source is now
    // accepted).
    ...
    // Chase: a $40 unplanned grocery charge, plus an internal
    // transfer to Amex ($200) which must be excluded.
    await db.insert(transactionsTable).values([
      {
        userId: TEST_USER,
        householdId: TEST_HOUSEHOLD_ID,
        occurredOn: "2026-05-19",
        occurredAt: "2026-05-19T08:00:00Z",
        description: "Groceries (Chase)",
        amount: "-40.00",
        plaidAccountId: chaseExternal,
        source: "plaid",
    ...
    // Amex: two grocery spends ($75 dinner-out + $30 small shop —
    // both in the Groceries bucket so the per-category aggregator
    // has to fold Chase+Amex together), a $25 refund (income), a
    // $10 row pointed at the excluded Transfer category (must drop
    // out via the #783 filter even though the source is now
    // accepted), plus a transfer-in row that must be excluded by
    // the new is_transfer guard.
    await db.insert(transactionsTable).values([
      {
        userId: TEST_USER,
        householdId: TEST_HOUSEHOLD_ID,
        ...
        description: "Dinner (Amex)",
        amount: "-75.00",
        plaidAccountId: amexExternal,
        source: "plaid:amex",
        categoryId: groceriesCat!.id,
      },
      {
      ...
      },
      {
        ...
        description: "Amex row in excluded bucket",
        amount: "-10.00",
        plaidAccountId: amexExternal,
        source: "plaid:amex",
        categoryId: excludedCat!.id,
      },
      {
        userId: TEST_USER,
        householdId: TEST_HOUSEHOLD_ID,
    ...
    const snap = await computeWeekVariance(TEST_HOUSEHOLD_ID, PRIOR_WEEK);
    // Income: $25 Amex refund only (the $200 transfer is excluded;
    // excluded-category row contributes nothing).
    expect(snap.totals.actualIncome).toBe("25.00");
    // Expenses: $40 Chase + $75 + $30 Amex = $145 (the $200
    // Chase→Amex transfer and the $10 excluded-category Amex row
    // both drop out).
    expect(snap.totals.actualExpenses).toBe("145.00");
    // 4 non-transfer, non-excluded rows surface as transactions.
    expect(snap.transactions).toHaveLength(4);
    const descs = snap.transactions.map((t) => t.description).sort();
    ...
    // Neither transfer row nor the excluded-category row leaks in.
    expect(
      snap.transactions.some((t) => /Payment from checking/i.test(t.description)),
    ).toBe(false);
    expect(
      snap.transactions.some((t) =>
        /excluded bucket/i.test(t.description),
      ),
    ).toBe(false);

    // byCategory: the Groceries bucket aggregates both Chase and
    // Amex rows ($40 + $75 + $30 + $25 = $170 absolute) — proves
    // the per-category accumulator no longer silently drops
    // categorized Amex spend. The excluded Transfer category must
    // not appear at all.
    const groceriesBucket = snap.byCategory.find(
      (b) => b.categoryId === groceriesCat!.id,
    );
    expect(groceriesBucket).toBeTruthy();
    expect(groceriesBucket!.actualAmount).toBe("170.00");
    expect(groceriesBucket!.actualTxns).toHaveLength(4);
    const bucketDescs = groceriesBucket!.actualTxns
      .map((t) => t.description)
      .sort();
    expect(bucketDescs).toEqual([
      "Amex refund",
      "Dinner (Amex)",
      "Gas (Amex)",
      "Groceries (Chase)",
    ...
    expect(
      snap.byCategory.some((b) => b.categoryId === excludedCat!.id),
    ).toBe(false);
  });
