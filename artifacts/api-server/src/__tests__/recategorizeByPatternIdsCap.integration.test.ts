...
import { recategorizeTransactionsByPatternBodyIdsMax } from "@workspace/api-zod";
...
describe("POST /transactions/recategorize-by-pattern id-list cap", () => {
  it("rejects payloads with more than the documented cap of ids with a 400 and a clear error message (no silent truncation)", async () => {
    const oversize = Array.from(
      { length: recategorizeTransactionsByPatternBodyIdsMax + 1 },
      () => randomUUID(),
    );
    ...
    expect(err).toContain(
      String(recategorizeTransactionsByPatternBodyIdsMax),
    );
  ...
  it("accepts a payload at exactly the cap (no off-by-one rejection)", async () => {
    const atCap = Array.from(
      { length: recategorizeTransactionsByPatternBodyIdsMax },
      () => randomUUID(),
    );
    const r = await api("POST", "/transactions/recategorize-by-pattern", {
      pattern: "anything",
