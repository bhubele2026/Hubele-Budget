...
// (#421) Pending-payment decrement. The reader moved to `lib/debtPending.ts`
// in C10 so `/api/spine` and `/api/dashboard` could net the same way this
// route always has — see that file's header. This route's behaviour is
// unchanged; it just imports what it used to define.
