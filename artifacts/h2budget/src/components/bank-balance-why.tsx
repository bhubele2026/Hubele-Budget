 ...
 *
 * Nothing is fetched until the popover opens (`enabled: open`), and nothing here
 * is computed: every figure and every reason comes from
 * `/forecast/bank-balance-explain`, which is read-only and makes no Plaid call.
 * The prose lives one tap away, where the word diet allows it.
 *
 ...
 * snapshot day. They follow different rules (PR4 makes cash today one rule), so
 * they are separate lines, and a note says so when they don't add up to the cent.
 */
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          className="press absolute right-2 top-2 rounded-control p-1 text-neutral-400 hover:text-brand-navy focus-visible:ring-2 focus-visible:ring-brand-navy/40"
      ...
      <PopoverContent className="w-80 p-3" align="end" data-testid="bank-why">
        {explain ? (
          ...
          <p className="text-micro text-neutral-500">Loading…</p>
        )}
      </PopoverContent>
    </Popover>
  );
...
function Explanation({ explain }: { explain: BankBalanceExplain }) {
  ...
  const addsUp =
    snapshot.balance != null && since != null
      ? cents(snapshot.balance) + cents(since.net) === cents(displayed.bankToday)
      : null;
  const where = [
    ...
    <div className="space-y-2">
      ...
      {snapshot.balance == null ? (
        ...
        <dl className="space-y-1 text-micro text-neutral-500">
          <div className="flex justify-between gap-2" data-testid="bank-why-snapshot">
            <dt>
              Snapshot{snapshot.at ? `, ${formatDate(householdDayOfAt(snapshot.at))}` : ""}
              {where ? ` (${where})` : ""}
            </dt>
      ...
      {addsUp === false && (
        <p className="text-micro text-neutral-500" data-testid="bank-why-mismatch">
          Those rows and the balance above are counted by different rules, so they do not add up
          to the cent.
        </p>
      )}
      ...
      <p className="text-micro text-neutral-500" data-testid="bank-why-next-sync">
        {nextSync.willRefreshBalance
          ? "The next Sync re-reads this balance from the bank."
          : `The next Sync won't re-read it: ${nextSync.whyNot ?? "no reason given"}.`}
      </p>
    </div>
  );
}
